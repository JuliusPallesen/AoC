...
int main(int argc, char const *argv[])
{
    ...
    for (size_t i = 0; i < input.size(); i++)
    {
        ans = std::max({
            ans,
            getIllumination(input, {i, 0}, Directions::Right),
            getIllumination(input, {i, input[0].size() - 1}, Directions::Left),
        });
    }
    ...
    for (size_t i = 0; i < input[0].size(); i++)
    {
        ans = std::max({
            ans,
            getIllumination(input, {0, i}, Directions::Down),
            getIllumination(input, {input.size() - 1, i}, Directions::Up),
        });
    }
