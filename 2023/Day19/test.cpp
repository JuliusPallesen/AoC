...
size_t _processPartWithWorkflow(const Part &part, const string &previousEvaluationResult, const vector<Rule> &remainingRules, const Input &input) {
    ...
    const auto splitSegmentParts = rule.split(part); // splits part in two depending on the rule value and op
    for (const auto &segmentPart : splitSegmentParts) {
        const auto evaluateOpt = rule.evaluate(segmentPart); // only runs code if valid (returns opt next step name)
        if (evaluateOpt) { // this if statement handles the accepting part / the rest
            const auto result = *evaluateOpt; // dereferences optional
            vector<Rule> newRules;
            if (result != "A" && result != "R") {
                const auto workflowName = result;
                newRules = input.workflowToRules.at(workflowName);
            }
            // calls process on the next step
